...
void OccupationMDrawComponent::drawObject()
{
	...
	for (int i = 0; i < MATRIX_X_SIZE; i++)
		for (int j = 0; j< MATRIX_Y_SIZE; j++)
		{
			...
			if ((robotMap->Map[i][j].prob.Empty > 0.51) || (robotMap->Map[i][j].prob.Empty < 0.49))
			//if (robotMap->Map[i][j].bSonarViewed == true)
			{
				glColor4f(robotMap->Map[i][j].prob.Empty, robotMap->Map[i][j].prob.Empty, robotMap->Map[i][j].prob.Empty, 1.0f);
				//glColor4f(1.0, 1.0, 1.0, 0.8f);
